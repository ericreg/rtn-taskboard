...
pub fn router(state: AppState) -> Router {
    let api = Router::new()
        ...
        .route(
            "/projects/{id}",
            get(projects::detail).patch(projects::update),
        )
        ...
        .route(
            ...
            put(tasks::watch).delete(tasks::unwatch),
        )
        .route("/tasks/{id}/links", post(tasks::link))
        .route("/tasks/{id}/links/{link}", delete(tasks::unlink))
        .route(
            "/tasks/{id}/attachments",
            post(media::task_upload).layer(DefaultBodyLimit::disable()),
        )
        .route(
            "/projects/{id}/attachments",
            post(media::project_upload).layer(DefaultBodyLimit::disable()),
        )
        .route(
            "/attachments/{id}",
